ssj_fix <- function(y, x = seq(1,length(y)), lambda, maxsum = sd(y, na.rm = TRUE)/mean(diff(x)),
  ...
  obj <- function(pars, wgt = FALSE) {
    ...
    list(
      ...
      gradient  = c(
        # w.r.t sigma_zeta
        -pars[1]*sum(rn_sum) - lambda*pars[1]*sum(e*e-d),
        # w.r.t. sigma_eta_t
...
auto_ssj_fix <- function(y, x = seq(1,length(y)), lambda,
