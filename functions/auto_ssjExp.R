## tuning di lambda:
auto_lambda_ssjExp <- function(y, x = seq(1,length(y)), #lambda_min=1e-6, lambda_max=1e12, #3000
                               maxsum = sd(y, na.rm = TRUE)/mean(diff(x)),
                               edf = TRUE, last_delta = 1,
                               alpha = NULL, beta = 10) {
  
  bic_objective <- function(loglambda, y, x, maxsum, edf, last_delta) {
    ssjExp_fix(y = y, x = x, lambda = exp(loglambda), maxsum = maxsum,
  ...
  if (is.null(alpha)) {
    temp <- ssjExp_fix(y = y, x = x, lambda = lambda_fin, maxsum = maxsum, #opt_lambda$minimum
  ...
  ssjExp_fix(y = y, x = x, lambda = lambda_fin, maxsum = maxsum, #opt_lambda$minimum
             edf = edf, last_delta = last_delta)
}

## tuning congiunto di M e lambda:
auto_ssjExp <- function(y, x = seq(1,length(y)), #lambda_min=1, lambda_max=1e4, #lambda_min=1e-6, lambda_max=1e12
  ...
  last_ic <- Inf
  for (M in grid) {
    out <- auto_lambda_ssjExp(y = y, x = x,
