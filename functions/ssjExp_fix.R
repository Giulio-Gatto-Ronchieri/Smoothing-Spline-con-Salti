ssjExp_fix <- function(y, x = seq(1,length(y)), lambda, maxsum = sd(y, na.rm = TRUE)/mean(diff(x)),
  ...
  obj <- function(pars, wgt = FALSE) {
    ...
    list(
      ...
      gradient  = c(
        # w.r.t sigma_zeta
        -pars[1]*sum(rn_sum1) - pars[1]*lambda*sum(e*e-d),
        # w.r.t. gamma_t
        -sig2/2*rn_sum2[-1]
      )/nobs
    )
  ...
  g <- function(pars, wgt) {
    list(
      ...
      jacobian = c(0, rep(1, n-1)) # derivatives of constr. w.t. to γ_t
    )
  }
  ...
  ## Starting values
  if (is.null(parinit)) {
    inits <- c(sd_zeta = sdy/10, rep(1, n-2), 0) #
  } else {
    inits <- parinit
  }
  ...
  lb <- c(quasizero, rep(0, n-1)) # lower bound
  ...
  if (edf == TRUE) {
    ...
    df <- 1 + sum(opt$solution[gamma_ndx] > quasizero) # come LASSO per lin reg
  }
...
auto_ssjExp_fix <- function(y, x = seq(1,length(y)), lambda,
  ...
  last_ic <- Inf
  for (M in grid) {
    out <- ssjExp_fix(y = y, x = x, lambda = lambda,
