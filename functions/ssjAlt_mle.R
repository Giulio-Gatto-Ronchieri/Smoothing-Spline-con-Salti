ssjAlt_mle <- function(#y, x, maxsum = sd(y),
                       y, x = seq(1,length(y)), maxsum = sd(y, na.rm = TRUE)/mean(diff(x)),
  ...
  obj <- function(pars, wgt = FALSE) {
    ...
    ga2 <- pars[3]*pars[3] # gamma square
    ...
    list(
      ...
      gradient  = c(
        # w.r.t sigma_zeta
        -pars[1]*sum(rn1*D[1, 1, ] +rn2*D[2, 2, ] +rn12*(2*si2+ga.p1*si2t)/root_ssg*D[1, 2, ]),
        # w.r.t sigma_epsilon
        -pars[2]*sum(e*e-d),
        # w.r.t gamma
        -pars[3]*sum(si2t*(rn12*root_ss/root_sg*D[1, 2, ] +rn2*D[2, 2, ])),
        # w.r.t. sigma_eta_t
  ...
  g <- function(pars, wgt) {
    list(
      ...
      jacobian = c(0, 0, 0, rep(1, n)) # derivatives of constr. w.t. to σ_t
    )
  }
  ...
  ## Starting values
  if (is.null(parinit)) {
    inits <- c(sd_zeta = sdy/10, sd_eps = sdy, sqrt_gamma = 1/10, rep(1, n-1), 0) #
  } else {
    inits <- parinit
  }
  ...
  lb <- c(quasizero, quasizero, quasizero, rep(0, n)) # lower bound
  ...
  if (edf == TRUE) {
    ...
    df <- 3 + sum(opt$solution[vt_eta_ndx] > quasizero) # come LASSO per lin reg
  }
  ...
  list(opt = opt,
       ...
       sigmas = c(NA, opt$solution[-c(1, 2, 3, n+3)]), # il primo è vuoto perché c'è un lag negli shock
...
auto_ssjAlt_mle <- function(y, x = seq(1,length(y)),
                            #grid = c(seq(0, sd(y, na.rm = TRUE)/mean(diff(x))*10, sd(y, na.rm = TRUE)/mean(diff(x))/10), seq(0, sd(y)*10, sd(y)/10)),
                            grid = seq(0, sd(y, na.rm = TRUE)/mean(diff(x))*10, sd(y, na.rm = TRUE)/mean(diff(x))/10), #101 punti
